...
$agentRoot = Join-Path $repoRoot "native\lwjgl2-patch-agent"
...
$asmJar = Join-Path $toolsDir "asm-5.2.jar"
$manifest = Join-Path $buildRoot "MANIFEST.MF"
$outJar = Join-Path $repoRoot "native\xbox-lwjgl2-patch-agent.jar"

New-Item -ItemType Directory -Force -Path $toolsDir, $classesDir, $jarRoot | Out-Null

if (-not (Test-Path $asmJar)) {
    $asmUrl = "https://repo1.maven.org/maven2/org/ow2/asm/asm/5.2/asm-5.2.jar"
    Write-Host "Downloading ASM bytecode library..."
...
Premain-Class: com.minecraftxbox.lwjgl2.Lwjgl2XboxAgent
